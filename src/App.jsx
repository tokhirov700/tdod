  ...
  return (
    <div className="container">
      ...
      <div className="form">
        ...
        <button onClick={clearAll} className="clearButton">Clear All</button>
      </div>
      
      {state.lastAddedTodo && (
        <div className="lastAdded">
          <h2>Last Added TODO</h2>
          <p className="title"><strong>Title:</strong> {state.lastAddedTodo.title}</p>
          <p className="time"> <strong>Time:</strong> {state.lastAddedTodo.time.toLocaleString()}</p>
        </div>
      )}
      
      {state.todos.map((todo, index) => (
        <div key={index} className={`card ${todo.completed ? 'completedCard' : ''}`}>
